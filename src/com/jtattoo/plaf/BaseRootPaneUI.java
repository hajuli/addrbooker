        ...
        public void mousePressed(MouseEvent ev) {
            ...
            if (getTitlePane() != null && getTitlePane().contains(convertedDragWindowOffset)) {
                if ((f != null && ((frameState & BaseRootPaneUI.MAXIMIZED_BOTH) == 0) || (d != null)) && dragWindowOffset.y >= BORDER_DRAG_THICKNESS && dragWindowOffset.x >= BORDER_DRAG_THICKNESS && dragWindowOffset.x < w.getWidth() - BORDER_DRAG_THICKNESS) {
                    ...
                    if (window instanceof JFrame) {
                        ...
                        for (int i = 0; i < pcl.length; i++) {
                            pcl[i].propertyChange(new PropertyChangeEvent(this, "windowMoving", Boolean.FALSE, Boolean.FALSE));
                        }
                    }
                ...
                if (window instanceof JFrame) {
                    ...
                    PropertyChangeListener[] pcl = frame.getPropertyChangeListeners();
                    for (int i = 0; i < pcl.length; i++) {
                        pcl[i].propertyChange(new PropertyChangeEvent(this, "windowResizing", Boolean.FALSE, Boolean.FALSE));
                    }
                }
                if (window instanceof JDialog) {
                    JDialog dialog = (JDialog) window;
        ...
        public void mouseReleased(MouseEvent ev) {
            ...
            if (w != null) {
                ...
                if (window instanceof JFrame) {
                    ...
                    for (int i = 0; i < pcl.length; i++) {
                        if (isMovingWindow) {
                            pcl[i].propertyChange(new PropertyChangeEvent(this, "windowMoved", Boolean.FALSE, Boolean.FALSE));
                        } else {
                            pcl[i].propertyChange(new PropertyChangeEvent(this, "windowResized", Boolean.FALSE, Boolean.FALSE));
                        }
                    }
                }
                if (window instanceof JDialog) {
